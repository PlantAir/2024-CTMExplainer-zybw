...
from ctmexplainer.xgraph.models.ext.tgat.graph import NeighborFinder
...
class AttnModel(torch.nn.Module):
    ...
    def __init__(
        self, feat_dim, edge_dim, time_dim, attn_mode="prod", n_head=2, drop_out=0.1
    ):
        """
        args:
    ...
    def tem_conv(
        ...
        if curr_layers == 0:
            ...
            src_ngh_feat = src_ngh_node_conv_feat.view(
                batch_size, self.num_neighbors, -1
            )
            ...
            if candidate_weights_dict is not None:
                ...
                mask = torch.zeros_like(src_ngh_node_batch_th).to(
                    dtype=torch.float32
                )  # NOTE: for +, 0 mean no influence
                # import ipdb; ipdb.set_trace()
            ...
            weight = weight.reshape(
                (
                    self.n_head,
                    src_node_batch_th.shape[0],
                    src_ngh_node_batch_th.shape[1],
                )
            )
