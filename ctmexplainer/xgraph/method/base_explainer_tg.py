...
from ctmexplainer.xgraph.method.tg_score import TGNNRewardWraper
from ctmexplainer.xgraph.dataset.utils_dataset import k_hop_temporal_subgraph
...
class BaseExplainerCTMS(object):
    def __init__(self,
                 model: Union[TGAN, None],
                 model_name: str,
                 explainer_name: str,
    ...
    def find_candidates(self, target_event_idx):
        ...
        if self.model_name in ['tgat', 'tgn']:
            ...
            u = self.all_events.iloc[
                target_event_idx - 1, 0]  # because target_event_idx should represent e_idx. e_idx = index + 1
            i = self.all_events.iloc[target_event_idx - 1, 1]
            ts = self.all_events.iloc[target_event_idx - 1, 2]
    ...
    def _set_candidate_events(self, event_idx):
        ...
        assert hasattr(self, 'ori_subgraph_df')
        # self.base_events = list(filter(lambda x: x not in candidate_events_set_, self.ori_subgraph_df.e_idx.values) ) # NOTE: ori_subgraph_df.e_idx.values
        self.base_events = list(filter(lambda x: x not in candidate_events_set_,
                                       unique_e_idx))  # NOTE: an importanct change, need test. largely influence the speed!
    ...
    def _save_candidate_scores(self, candidate_weights, event_idx, runtimes):
        ...
        only for baseline explainer, save their computed candidate scores.
        """
        filename = self._score_path(self.results_dir, self.model_name, self.dataset_name, self.explainer_name,
                                    self.navigator_type, event_idx, self.threshold_num)
        data_dict = {
            'candidates': [],
            "runtime": runtimes
        }
